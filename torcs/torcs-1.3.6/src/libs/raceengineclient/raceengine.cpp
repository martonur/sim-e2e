...
reCapture(void)
{
    ...
    std::cout << capture->currentFrame / 2 << std::endl;
    capture->currentFrame++;

    // Declare and define image parameters
	unsigned char *img;
	int sw, sh, vw, vh;
    ...
    // Get camera image with OpenGL
	glPixelStorei(GL_PACK_ROW_LENGTH, 0);
	glPixelStorei(GL_PACK_ALIGNMENT, 1);
	glReadBuffer(GL_FRONT);
	glReadPixels((sw-vw)/2, (sh-vh)/2, vw, vh, GL_RGB, GL_UNSIGNED_BYTE, (GLvoid*)img);

    // Send image to neural network through socket
	std::string image_data((const char*)img,imgsize);
	int client_socket = ReInfo->socket;
    send(client_socket , image_data.c_str() , imgsize , 0 );

    free(img);


    // Uncomment following block comment to start wheel steering angle extraction
    /*
	// Append current steering angle to steering.txt at ~12.5 Hz
	const int FILENAMESIZE = 1024;
	char filename[FILENAMESIZE];
	snprintf(filename, FILENAMESIZE, "%s/steering.txt", capture->outputBase);
	std::ofstream file;
	file.open (filename, std::ios_base::app);
	file << ReInfo->s->cars[0]->_steerCmd << " " << ReInfo->s->cars[0]->_accelCmd << " " << ReInfo->s->cars[0]->_brakeCmd << " " <<  capture->currentFrame / 2 << std::endl;
	file.close();
	*/
...
ReUpdate(void)
{
	...
	switch (ReInfo->_displayMode) {
		...
		case RM_DISP_MODE_CAPTURE:
			...
			GfuiDisplay();
			// Change capture rate from 25 Hz to 12.5 Hz
			ReInfo->_reGraphicItf.refresh(ReInfo->s);
			if(capture->currentFrame % 2 == 0) {
				reCapture();
			}
			else {
				capture->currentFrame++;
