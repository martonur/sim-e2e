...
void SetupSocket() {
    int file_descriptor, new_socket;
	...
	if ((file_descriptor = socket(AF_INET, SOCK_STREAM, 0)) == 0) {
		std::cerr << "File descriptor error" << std::endl;
		exit(1);
	}

	// Forcefully attaching socket, preventing "Address already in use" error
	if (setsockopt(file_descriptor, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT, &opt, sizeof(opt))) {
        std::cerr << "Attachment error" << std::endl;
		exit(1);
	}
	...
	if (bind(file_descriptor, (struct sockaddr *)&socket_address, sizeof(socket_address)) < 0) {
        std::cerr << "Binding error" << std::endl;
		exit(1);
	}
	std::cout << "The simulation server is bound to the socket and is waiting for a client!" << std::endl;
	// Put server socket in a passive mode, waiting for client
	...
	// Extract first connection request, create new connected socket, return file descriptor referring to the socket
	if ((new_socket = accept(file_descriptor, (struct sockaddr *)&socket_address, (socklen_t*)&addrlen)) < 0) {
        std::cerr << "Accept error" << std::endl;
		exit(1);
	}

	std::cout << "Connection established between client and server, ready for data transfer" << std::endl;
    ReInfo->socket = new_socket;
}
