...
void
reSelectRaceman(void *);

// Skips the main menu and jumps to starting practice race
// Reads selected map from config file
void
StartRace(int)
{
	GfuiScreenActivate(ReSinglePlayerInit(menuHandle));
	void *params = GfParmReadFile("/root/.torcs/config/raceman/practice.xml", GFPARM_RMODE_STD);
